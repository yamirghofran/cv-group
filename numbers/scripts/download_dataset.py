...
def convert(raw_dir: Path, out_dir: Path) -> dict[str, int]:
    ...
    for raw_split, out_split in SPLIT_MAP.items():
        ...
        dst_images_dir = out_dir / out_split
        dst_images_dir.mkdir(parents=True, exist_ok=True)
        out_jsonl = out_dir / f"{out_split}.jsonl"

        print(f"  Converting {raw_split} to {out_split} using {jsonl_file.name}")

        n = 0
        with jsonl_file.open(encoding="utf-8") as fin, out_jsonl.open("w", encoding="utf-8") as fout:
            for line in fin:
                line = line.strip()
                if not line:
                    continue
                ...
                PILImage.open(src).convert("RGB").resize((224, 224)).save(str(dst))
                fout.write(json.dumps({"image": str(dst.resolve()), "prompt": PROMPT, "answer": label}) + "\n")
                n += 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("\nConverting JSONL → VQA JSONL structure ...")
    counts = convert(raw_dir, out_dir)

    print(f"\nDone. Dataset at: {out_dir}")
    for split, n in counts.items():
        print(f"  {split}: {n} entries")
